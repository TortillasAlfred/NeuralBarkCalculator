...
def main():
    ...
    train_loader = DataLoader(Subset(train_dataset, train_split.repeat(25)), batch_size=16, shuffle=True)
    ...
    if not os.path.isdir("/mnt/storage/mgodbout/Ecorcage/Images/results/cwce_56"):
        os.makedirs("/mnt/storage/mgodbout/Ecorcage/Images/results/cwce_56")
    ...
    with torch.no_grad():
        for batch, pure_batch in zip(valid_loader, pure_loader):
            ...
            # if os.path.isfile("/mnt/storage/mgodbout/Ecorcage/Images/results/cwce_56/{}".format(fname)):
            #     continue
