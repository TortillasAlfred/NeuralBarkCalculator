...
def generate_output_folders(root_dir):
    ...
    results_dir = os.path.join(root_dir, 'Images', 'results', 'big_aug')
...
def get_loader_for_crop_batch(crop_size, batch_size, train_split, mean, std, train_weights, root_dir):
    train_dataset = RegressionDatasetFolder(os.path.join(root_dir, "Images/generated_exp"),
                                            input_only_transform=Compose([
                                                ToPILImage(),
                                                ColorJitter(brightness=0.15, saturation=0.3),
                                                ToTensor(),
                                                Normalize(mean, std)
                                            ]),
                                            transform=Compose([
...
def main(args):
    ...
    exp = Experiment(directory=os.path.join(args.root_dir, 'big_aug/'),
                     module=module,
    ...
    with torch.no_grad():
        for image_number, (batch, pure_batch) in enumerate(zip(valid_loader, pure_loader)):
            ...
            # if os.path.isfile('/mnt/storage/mgodbout/Ecorcage/Images/results/big_aug/{}'.format(fname)):
            #     continue
            ...
            plt.savefig(os.path.join(args.root_dir,
                                     'Images/results/big_aug/combined_images/{}/{}/{}').format(wood_type, split, fname),
                        format='png',
                        dpi=900)
            plt.close()
            ...
            dual.save(
                os.path.join(args.root_dir, 'Images/results/big_aug/outputs/{}/{}/{}').format(wood_type, split, fname))

            results_csv.append(running_csv_stats)

    csv_file = os.path.join(args.root_dir, 'Images', 'results', 'big_aug', 'final_stats.csv')
