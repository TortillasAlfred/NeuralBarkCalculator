...
def new_new_main():
    ...
    train_loader = DataLoader(Subset(train_dataset, train_split), batch_size=1, shuffle=True)
    valid_loader = DataLoader(Subset(valid_dataset, valid_split), batch_size=1)
    test_loader = DataLoader(Subset(test_dataset, test_split), batch_size=1)

    module = vanilla_unet()
    optim = torch.optim.Adam(
        module.parameters(), lr=1e-3, weight_decay=1e-5)
    exp = Experiment(directory="./dual_unet/",
                     module=module,
                     device=torch.device("cuda:0"),
                     optimizer=optim,
    ...
    for batch, pure_batch in zip(valid_loader, pure_loader):
        ...
        for i in range(batch[1].size(0)):
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig("./Images/results/dual_unet/{}".format(batch[3][i]),
                        format="png",
                        dpi=900)
